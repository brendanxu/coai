  ...
  return (
    ...
    >
      ...
      <div className="callout">
        <p>
          <strong>BYOK ≠ 中转(当前技术状态):</strong>我们当前在应用层 不存储
          prompt / response 内容,仅记录元数据(模型名、token 数、
          时间戳)用于计费。这描述的是当前实现状态,不是永久承诺 — 配置可在仓库{" "}
          <code>infra/coai-config.yaml.example</code> + NewAPI dashboard
          验证;任何变更会同步更新 <a href="/privacy">隐私政策</a>。
        </p>
      </div>
      ...
      <ol>
        <li>
          注册并购买任一 Token 套餐（详见 <a href="/token-plans">Token 套餐</a>
          ）
        </li>
        <li>
      ...
      <p>
        所有 API 请求都需要在 <code>Authorization</code> 头里带上你的 sk-key：
      </p>
      <pre>
      ...
      <ul>
        ...
        <li>
          Key 不会过期；套餐到期后调用会返回 <code>402 quota_exhausted</code>
        </li>
        <li>
      ...
      <p>
        ...
        <code>GET /v1/models</code> 拿到你的套餐当前能用的全部模型 + 当前 系数。
      </p>
      ...
      <pre>
        <code>
          credits 消耗 = (input_tokens + output_tokens × 4) × 系数 / 1500
        </code>
      </pre>
      ...
      <h3>支持哪些上游模型？</h3>
      <p>
        OpenAI / Anthropic / Google / DeepSeek / Qwen / 智谱 / Moonshot / 百川
        等。完整列表见运行时 <code>GET /v1/models</code>，或参考{" "}
        <a href="/pool">模型池</a> 页面。
      </p>

      <h3>能开发票吗？</h3>
      <p>
        可以。月度结账后联系 <a href="/contact">support</a>{" "}
        索取增值税普通发票（电子）。 企业台头需提前提供税号。
      </p>
...
function DocsEN() {
  return (
    ...
    >
      <p>
        The greentokey Token Plan (Layer 2) is an OpenAI-compatible API gateway.
        Buy a plan, get an <code>sk-xxx</code> key, and call GPT, Claude,
        DeepSeek, Gemini and others through one endpoint. No code changes — just
        point your client&apos;s <code>base_url</code> at us.
      </p>

      <div className="callout">
        <p>
          <strong>BYOK ≠ proxying (current technical state):</strong> under the
          current configuration we do not store prompts or completions at the
          application layer; we log only metadata (model, token count,
          timestamp) for billing. This describes current implementation, not a
          perpetual commitment — configuration is verifiable in the repository (
          <code>infra/coai-config.yaml.example</code>) and the NewAPI dashboard,
          and any change will be reflected in the{" "}
      ...
      <ol>
        <li>
          Sign up and buy any plan on <a href="/token-plans">Token Plans</a>
        </li>
        <li>
      ...
      <ul>
        <li>
          Each plan order generates a separate key (good for project isolation +
          audit)
        </li>
        <li>
          Keys don&apos;t expire; calls return <code>402 quota_exhausted</code>{" "}
          when the plan runs out
        </li>
        <li>
          If a key leaks, revoke and regenerate from the{" "}
          <a href="/account">account page</a>
        </li>
      </ul>
      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            <td>Chat completion (streaming supported)</td>
          </tr>
          <tr>
            <td>
              <code>POST /v1/embeddings</code>
          ...
          <tr>
            ...
            <td>List models available on your plan</td>
          </tr>
        </tbody>
      </table>
      ...
      <p>
        Models are grouped into 3 tiers by upstream cost. 1 credit translates to
        different effective quota across tiers:
      </p>
      <table>
        <thead>
          <tr>
      ...
      <pre>
        <code>
          credits = (input_tokens + output_tokens × 4) × multiplier / 1500
        </code>
      </pre>
      <p>Response headers on every call:</p>
      <ul>
        <li>
          <code>X-Quota-Used</code> — quota spent on this call
        </li>
        <li>
          <code>X-Quota-Remaining</code> — quota left on the plan
        </li>
        <li>
          <code>X-Quota-Tier</code> — tier the model resolved to
        </li>
      </ul>
      <p>
        At &lt; 10% remaining we surface a banner on the account page. At 0 we
        return 402 — we never silently double-bill upstream.
      </p>
      ...
      <p>
        ...
        set their OpenAI Base URL to <code>https://api.greentokey.com/v1</code>.
      </p>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>Upstream temporarily down — retry per retry-after</td>
          </tr>
        </tbody>
      </table>
      ...
      <p>
        Need more? Email <a href="/contact">support</a>.
      </p>

      <h2 id="faq">FAQ</h2>
      <h3>Will Chinese network conditions affect me?</h3>
      <p>
        api.greentokey.com is fronted by Cloudflare and reaches most Chinese
        ISPs reliably. Special-case access (campus networks, etc.) can use{" "}
        <code>cn.api.greentokey.com</code> on request.
      </p>
      <h3>Which upstream models are supported?</h3>
      <p>
        OpenAI / Anthropic / Google / DeepSeek / Qwen / Zhipu / Moonshot /
        Baichuan and more. Live list via <code>GET /v1/models</code> or the{" "}
        <a href="/pool">model pool</a> page.
      </p>
      <h3>Can I get an invoice?</h3>
      <p>
        Yes. Email <a href="/contact">support</a> after checkout for an
        electronic VAT invoice (Chinese 增值税普通发票). Provide the tax ID up
        front for company titles.
      </p>
